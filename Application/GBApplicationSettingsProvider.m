...
- (NSString *)stringByReplacingOccurencesOfRegex:(NSString *)regex inHTML:(NSString *)string usingBlock:(NSString *(^)(NSInteger captureCount, NSString * const capturedStrings[captureCount], BOOL insideCode))block;
...
- (NSString *)stringByConvertingMarkdownToHTML:(NSString *)markdown {
	...
	if (self.embedCrossReferencesWhenProcessingMarkdown) {
		...
		result = [self stringByReplacingOccurencesOfRegex:regex inHTML:result usingBlock:^NSString *(NSInteger captureCount, NSString *const __unsafe_unretained *capturedStrings, BOOL insideCode) {
...
- (NSString *)stringByReplacingOccurencesOfRegex:(NSString *)regex inHTML:(NSString *)string usingBlock:(NSString *(^)(NSInteger captureCount, NSString * const capturedStrings[captureCount], BOOL insideCode))block {
	...
	return [string stringByReplacingOccurrencesOfRegex:theRegex usingBlock:^NSString *(NSInteger captureCount, NSString *const __unsafe_unretained *capturedStrings, const NSRange *capturedRanges, volatile BOOL *const stop) {
		...
		return block(captureCount, capturedStrings, insideExampleBlock);
	}];
}
